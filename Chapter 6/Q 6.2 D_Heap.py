...
# 6.2(a)：如何在一个数组中表示一个 d 叉堆。 答：在Python中，Parent 下标为(i-1)//d，Children下标为 d*i + c + 1（其中 0 <= c < d） 
# 6.2(b)：包含 n 个元素的 d 叉堆的高度是多少？请用 n 和 d 表示。  答：以 d 为底 n 的对数 logd(n) 
# 6.2(c)：请给出 Extract-Max 在 d 叉最大堆上的一个有效实现，并用 d 和 n 表示出它的时间复杂度。 O(dlogd(n))
# 6.2(d)：给出 Insert 在 d 叉最大堆上的一个有效实现，并用 d 和 n 表示出它的时间复杂度。O(logd(n))
# 6.2(e)：给出 Increase-Key(A, i, k) 的一个有效实现，当 k < A[i] 时，它会触发一个错误，否则执行 A[i] = k，并更新相应的 d 叉最大堆，
#         请用 d 和 n 表示出它的时间复杂度。O(logd(n))
